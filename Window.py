...
class Window:
    def __init__(self,size:tuple[int,int]):
        display.init()
        display.set_mode(size,const.OPENGL|const.DOUBLEBUF)
        ...
        self.program, self.render_object = self.getShader('./shaders/everything.vert','./shaders/game.frag')
    ...
    def getShader(self,vertex_path:str,fragment_path:str,size = (1.0,1.0),offset = (0.0,0.0)):
        quad_buffer = _makeBuffer(self.ctx,size,offset)
        program = self.ctx.program(vertex_shader=_readShader(vertex_path), fragment_shader=_readShader(fragment_path))
        render_object = self.ctx.vertex_array(program,[(quad_buffer,'2f 2f', 'vert', 'texcoord')],mode = moderngl.TRIANGLE_STRIP)
